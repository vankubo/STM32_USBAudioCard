...
u8 WM8978_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOA, ENABLE);			//使能外设GPIOB,GPIOA时钟
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化
	...
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource6,GPIO_AF6_SPI2);	//PC6 ,AF5  I2S_MCK
	...
	//以下为通用设置
	WM8978_Write_Reg(1,0X1B|(1<<5));	//R1,MICEN设置为1(MIC使能),BIASEN设置为1(模拟器工作),VMIDSEL[1:0]设置为:11(5K)
	WM8978_Write_Reg(2,0X1B0);	//R2,ROUT1,LOUT1输出使能(耳机可以工作),BOOSTENR,BOOSTENL使能
	WM8978_Write_Reg(3,0X6C);	//R3,LOUT2,ROUT2输出使能(喇叭工作),RMIX,LMIX使能	
/*
时钟设置
使用PLL,MCLK=12MHz，REQUIRE=12.288MHz
R6=DEFAULT，MCLK由外部提供，使用PLL分频
*/
	WM8978_Write_Reg(36,0X8);		//R36[3:0],PLLN=0X8
	
	temp=(0x3126E9>>18)&0x3f;
	WM8978_Write_Reg(37,temp);		//R37[5:0],PLLK[23:18]
	//printf("pllk1:0x%X\n",temp);
	
	temp=(0x3126E9>>9)&0x1ff;
	WM8978_Write_Reg(38,temp);		//R38[8:0],PLLK[17:9]
	//printf("pllk2:0x%X\n",temp);
	
	temp=(0x3126E9)&0x1ff;
	WM8978_Write_Reg(39,temp);		//R39[8:0],PLLK[8:0]
	//printf("pllk3:0x%X\n",temp);
